...
import { Col, Row, Card, Image, Button, Table, ProgressBar } from '@themesberg/react-bootstrap';
...
export const TransactionsTable = (props) => {
  ...
  const TableRow = (props) => {
    const { products, day, sales, profit } = props;

    return (
      <tr>
        <td>
          <img src={products} alt="product" width="100" height="150" />
        </td>
        <td>
        ...
        <Table hover className="user-table align-items-center">
          <thead>
            <tr>
              ...
              <th className="border-bottom">Launch Date</th>
              <th className="border-bottom">Sales</th>
              <th className="border-bottom">Profit</th>
            </tr>
          </thead>
          <tbody>
            {data.map(t => <TableRow key={`transaction-${t.products}`} {...t} />)}
          </tbody>
        </Table>
      </Card.Body>
    </Card>
